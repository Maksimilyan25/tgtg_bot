...
export async function removeAccountInteractive() {
    ...
    return await removeAccountNonInteractive(acc.id);
}

/**
 * Удаляет аккаунт без интерактивного подтверждения
 * @param {string} accountId - ID аккаунта для удаления
 * @param {boolean} forceConfirm - Автоматическое подтверждение удаления
 * @returns {Object} - Результат операции в формате JSON
 */
export async function removeAccountNonInteractive(accountId, forceConfirm = false) {
    if (!accountId) {
        return {
            success: false,
            message: 'Не указан ID аккаунта для удаления'
        };
    }

    const tokens = loadTokens();
    const accountExists = tokens.some(token => token.id === accountId);

    if (!accountExists) {
        return {
            success: false,
            message: `Аккаунт ${accountId} не найден`
        };
    }
    ...
    if (!forceConfirm) {
        return {
            success: false,
            message: 'Требуется подтверждение удаления (используйте --force)'
        };
    }
    ...
    return {
        success: true,
        message: `Аккаунт ${accountId} удалён`,
        accountId: accountId
    };
}